  ...
  return (
    <div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
        <h3 style={{fontSize:20,fontWeight:800,color:"var(--text-primary)"}}>{t("profili.title")}</h3>
        <div style={{display:"flex",gap:8}}>
          ...
          <button onClick={()=>onOpenForm()} className="s3d-btn s3d-btn-primary">{t("profili.nuovo")}</button>
        </div>
      </div>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16}}>
        ...
        <input type="text" value={search} onChange={e=>setSearch(e.target.value)} placeholder={t("profili.searchPlaceholder")} className="s3d-input" style={{flex:1}}/>
      </div>
      <div className="s3d-card" style={{overflow:"hidden"}}>
        {filtered.length===0?(
          <div style={{padding:"48px 0",textAlign:"center",color:"var(--text-muted)"}}>{items.length===0?t("profili.noProfili"):t("common.noResults")}</div>
        ):(
          <table className="s3d-table"><thead><tr style={{background:"var(--bg-surface)",borderBottom:"1px solid var(--border-default)"}}>
            <th style={{width:40}}></th><th>{t("common.name")}</th><th>{t("profili.thLayer")}</th><th>{t("profili.thPareti")}</th><th>{t("profili.thInfill")}</th><th>{t("profili.thTopBot")}</th><th>{t("profili.thSupporti")}</th><th style={{textAlign:"right"}}>{t("common.actions")}</th>
            ...
            <tr key={p.id} style={{cursor:"pointer"}} onClick={()=>onOpenForm(p.id)}>
              ...
              <td>
                {p.supporti_albero && <span style={{fontSize:10,padding:"2px 6px",background:"var(--green-soft)",color:"var(--green)",borderRadius:4,marginRight:4}}>{t("profili.supportiAlbero")}</span>}
                {p.supporti_normali && <span style={{fontSize:10,padding:"2px 6px",background:"var(--orange-soft)",color:"var(--orange)",borderRadius:4}}>{t("profili.supportiNormali")}</span>}
                {!p.supporti_albero && !p.supporti_normali && <span style={{color:"var(--text-muted)"}}>-</span>}
              </td>
              <td style={{textAlign:"right"}} onClick={e=>e.stopPropagation()}>
                <button onClick={()=>onOpenForm(p.id)} style={{background:"none",border:"none",color:"var(--accent)",fontSize:12,fontWeight:600,cursor:"pointer",marginRight:12}}>{t("common.edit")}</button>
                <button onClick={()=>handleDelete(p.id)} style={{background:"none",border:"none",color:"var(--red)",fontSize:12,fontWeight:600,cursor:"pointer"}}>{t("common.delete")}</button>
              </td>
            </tr>
          );})}</tbody></table>
        )}
