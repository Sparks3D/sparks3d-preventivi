        ...
        {filtered.length===0?(
          ...
          <table className="s3d-table"><thead><tr style={{background:"var(--bg-surface)",borderBottom:"1px solid var(--border-default)"}}>
            ...
            <tr key={m.id} style={{cursor:"pointer"}} onClick={()=>onOpenForm(m.id)}>
              ...
              <td><div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:10,padding:"2px 8px",background:o.bg,color:o.fg,borderRadius:4,fontWeight:600}}>{o.label}</span><span style={{fontWeight:600,color:"var(--text-primary)"}}>{m.nome}</span></div></td>
