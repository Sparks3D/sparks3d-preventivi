    ...
    <div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
        ...
        <div style={{display:"flex",gap:8}}>
          ...
          <button onClick={()=>onOpenForm()} className="s3d-btn s3d-btn-primary">Nuova stampante</button>
        </div>
      </div>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16,padding:12,background:"rgba(249,115,22,0.08)",border:"1px solid rgba(249,115,22,0.2)",borderRadius:10}}>
        <span className="text-sm font-medium text-amber-800 dark:text-amber-300">Costo energia €/kWh</span>
      ...
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16}}>
        ...
        <input type="text" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Cerca stampante" className="s3d-input" style={{flex:1}}/>
      </div>
      <div className="s3d-card" style={{overflow:"hidden"}}>
        {filtered.length===0?(
          <div style={{padding:"48px 0",textAlign:"center",color:"var(--text-muted)"}}>{items.length===0?'Usa "Importa da Bambu Studio", "Importa da Orca Slicer" o "Nuova stampante".':"Nessun risultato."}</div>
        ):(
          <table className="s3d-table"><thead><tr style={{background:"var(--bg-surface)",borderBottom:"1px solid var(--border-default)"}}>
            <th style={{width:40}}></th><th>Nome</th><th>Consumo kW/h</th><th>Ammortamento/h</th><th style={{textAlign:"right"}}>Azioni</th>
