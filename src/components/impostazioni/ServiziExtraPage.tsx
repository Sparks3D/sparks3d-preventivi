  ...
  return (
    <div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
        ...
        <button onClick={()=>onOpenForm()} className="s3d-btn s3d-btn-primary">+ Nuovo servizio</button>
      </div>
      <div className="s3d-card-glow p-5 mb-5">
        ...
        <div className="flex flex-wrap gap-2">
          {PRESETS.map(p=>(<button key={p.nome} onClick={()=>onOpenForm(undefined,p)} className="s3d-btn s3d-btn-ghost text-xs">+ {p.nome} ({eur(p.importo_predefinito)})</button>))}
        </div>
      </div>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16}}>
        ...
        <input type="text" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Cerca servizio extra" className="s3d-input" style={{flex:1}}/>
      </div>
      <div className="s3d-card" style={{overflow:"hidden"}}>
        {filtered.length===0?(
          ...
          <table className="s3d-table"><thead><tr style={{background:"var(--bg-surface)",borderBottom:"1px solid var(--border-default)"}}>
            <th style={{width:40}}></th><th>Servizio</th><th>Costo base</th><th>Markup</th><th>→ Cliente</th><th>Addebito</th><th style={{textAlign:"right"}}>Azioni</th>
            ...
            <tr key={s.id} style={{cursor:"pointer"}} onClick={()=>onOpenForm(s.id)}>
              ...
              <td>{s.addebita?<span className="s3d-badge" style={{background:"var(--green-soft)",color:"var(--green)"}}>Sì, al cliente</span>:s.addebita_senza_costo?<span className="s3d-badge" style={{background:"var(--orange-soft)",color:"var(--orange)"}}>Visibile, no costo</span>:<span style={{color:"var(--text-muted)",fontSize:12}}>Costo interno</span>}</td>
              <td style={{textAlign:"right"}} onClick={e=>e.stopPropagation()}>
                <button onClick={()=>onOpenForm(s.id)} style={{background:"none",border:"none",color:"var(--accent)",fontSize:12,fontWeight:600,cursor:"pointer",marginRight:12}}>Modifica</button>
