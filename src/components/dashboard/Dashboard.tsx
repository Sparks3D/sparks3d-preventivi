...
const V = {
  ...
  card: "#1a2340",
  cardHover: "#1f2b4d",
  border: "rgba(99,180,255,.15)",
  borderLight: "rgba(99,180,255,.25)",
  text: "#ffffff",
  textSec: "#b8c5db",
  textMuted: "#6b7fa0",
  accent: "#0ea5e9",
  accentGlow: "rgba(14,165,233,.12)",
  green: "#22c55e",
  greenGlow: "rgba(34,197,94,.12)",
  orange: "#f97316",
  orangeGlow: "rgba(249,115,22,.12)",
  pink: "#ec4899",
  pinkGlow: "rgba(236,72,153,.12)",
  cyan: "#22d3ee",
  cyanGlow: "rgba(34,211,238,.12)",
  purple: "#a855f7",
  purpleGlow: "rgba(168,85,247,.12)",
  red: "#f43f5e",
};

const STATO_COLORS: Record<string, { bg: string; text: string; dot: string }> = {
  bozza:          { bg: "rgba(148,163,184,.12)", text: "#94a3b8", dot: "#64748b" },
  non_confermato: { bg: "rgba(249,115,22,.12)",  text: "#fdba74", dot: "#f97316" },
  confermato:     { bg: "rgba(34,197,94,.12)",   text: "#86efac", dot: "#22c55e" },
  in_lavorazione: { bg: "rgba(14,165,233,.12)",  text: "#7dd3fc", dot: "#0ea5e9" },
  completato:     { bg: "rgba(168,85,247,.12)",  text: "#d8b4fe", dot: "#a855f7" },
  annullato:      { bg: "rgba(244,63,94,.12)",   text: "#fda4af", dot: "#f43f5e" },
};
const STATO_LABELS: Record<string, string> = { bozza:"Bozza",non_confermato:"Non confermato",confermato:"Confermato",in_lavorazione:"In lavorazione",completato:"Completato",annullato:"Annullato" };
  ...
  return (
    <div style={{ maxWidth: 1400, margin: "0 auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 20 }}>
        ...
        <KpiCard label="Fatturato" value={eur(stats.fatturato_totale)} sub={`Media ${eur(stats.valore_medio)}`} accent={V.green} glow={V.greenGlow} icon="💰" />
        <KpiCard label="Profitto" value={eur(stats.profit_totale)} sub={`Conversione ${num(stats.tasso_conversione, 0)}%`} accent={V.pink} glow={V.pinkGlow} icon="📈" />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 16, marginBottom: 20 }}>
        ...
        <StatiBreakdown data={stats.per_stato} totale={stats.totale_preventivi} />
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 16 }}>
        <UltimiPreventivi data={stats.ultimi_preventivi} />
...
function KpiCard({ label, value, sub, accent, glow, icon }: { label: string; value: string; sub: string; accent: string; glow: string; icon: string; }) {
  const [h, setH] = useState(false);
  return (
    <div style={{ ...sty.card, borderColor: h ? accent : V.border, background: h ? glow : V.card, transition: "all .2s" }} onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}>
...
function StatiBreakdown({ data, totale }: { data: StatoCount[]; totale: number }) {
  return (<div style={sty.card}><Sh>Stato preventivi</Sh>
    <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 16 }}>
      {data.map(s => { const pct = totale > 0 ? (s.count / totale) * 100 : 0; const sc = STATO_COLORS[s.stato] || STATO_COLORS.bozza;
        return (<div key={s.stato}><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}><div style={{ width: 8, height: 8, borderRadius: "50%", background: sc.dot }} /><span style={{ fontSize: 13, color: V.text, fontWeight: 500 }}>{STATO_LABELS[s.stato] || s.stato}</span></div>
          <div style={{ display: "flex", gap: 12 }}><span style={{ fontSize: 12, color: V.textSec }}>{s.count}</span><span style={{ fontSize: 12, color: sc.text, fontWeight: 600, minWidth: 70, textAlign: "right" }}>{eur(s.totale)}</span></div>
        </div><div style={{ height: 5, background: "rgba(255,255,255,.06)", borderRadius: 3, overflow: "hidden" }}><div style={{ height: "100%", width: `${pct}%`, background: sc.dot, borderRadius: 3, opacity: .7 }} /></div></div>);
      })}
    </div>
  </div>);
}

function UltimiPreventivi({ data }: { data: PreventivoRecente[] }) {
  return (<div style={sty.card}><Sh>Ultimi preventivi</Sh>
    <div style={{ marginTop: 12, overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse" }}>
      ...
      <tbody>{data.map(p => { const sc = STATO_COLORS[p.stato] || STATO_COLORS.bozza; return (
        <tr key={p.id} style={sty.tr}>
          <td style={sty.td}><span style={{ fontWeight: 600, color: V.accent }}>{p.numero}</span></td>
          <td style={sty.td}><span style={{ color: p.cliente_nome ? V.text : V.textMuted }}>{p.cliente_nome || "—"}</span></td>
          <td style={{ ...sty.td, fontVariantNumeric: "tabular-nums" }}>{p.data_creazione}</td>
          <td style={sty.td}><span style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "3px 10px", borderRadius: 20, fontSize: 11, fontWeight: 600, background: sc.bg, color: sc.text }}><span style={{ width: 6, height: 6, borderRadius: "50%", background: sc.dot }} />{STATO_LABELS[p.stato] || p.stato}</span></td>
...
function TopClienti({ data }: { data: TopCliente[] }) {
  ...
  return (<div style={sty.card}><Sh>Top clienti</Sh><div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 12 }}>
    {data.map((c, i) => (<div key={c.cliente_id} style={{ display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13, fontWeight: 500, color: V.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c.nome_display}</div>
        <div style={{ height: 3, background: "rgba(255,255,255,.06)", borderRadius: 2, marginTop: 4 }}><div style={{ height: "100%", borderRadius: 2, width: `${(c.fatturato / max) * 100}%`, background: `linear-gradient(90deg, ${V.accent}, ${V.cyan})`, opacity: .6 }} /></div></div>
